(function (namespace, $) {
	...
	p._handleChatMessage = function (e) {
		...
		if (e.keyCode === 13) {
			...
			// Get chat message
			var CodeigniterTime = new Date().getHours() + ':' + new Date().getMinutes();
			var CodeigniterImage = $('.list-chats li img').attr('src');
			
			// Create html
			var html = '';
			html += '<li>';
			html += '	<div class="chat">';
			html += '		<div class="chat-avatar"><img class="img-circle" src="' + CodeigniterImage + '" alt=""></div>';
			html += '		<div class="chat-body">';
			html += '			' + input.val();
			html += '			<small>' + CodeigniterTime + '</small>';
			html += '		</div>';
			html += '	</div>';
			html += '</li>';
			var $new = $(html).hide();
	...
	// =========================================================================
	namespace.Codeigniter = new Codeigniter;
